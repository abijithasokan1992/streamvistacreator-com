...
BEGIN
  ...
  -- 1. creator A edits creator B title (RLS 0 rows)
  PERFORM set_config('request.jwt.claims',json_build_object('sub',p_creator_a::text,'role','authenticated')::text,true);
  UPDATE public.content_titles SET title=title WHERE id=p_title_b;
  GET DIAGNOSTICS n = ROW_COUNT;
  RETURN QUERY SELECT 'creator_A_edits_B_title'::text,
    CASE WHEN n=0 THEN 'PASS' ELSE 'FAIL' END, format('rows=%s',n);

  -- 2. creator draft -> published
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'creator_draft_to_published','PASS',SQLERRM; END;

  -- 3. creator writes published_by
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'creator_writes_published_by','PASS',SQLERRM; END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'creator_reassign_owner','PASS',SQLERRM; END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'creator_change_workspace','PASS',SQLERRM; END;

  -- 6. admin draft -> published
  PERFORM set_config('request.jwt.claims',json_build_object('sub',p_admin::text,'role','authenticated')::text,true);
  BEGIN UPDATE public.content_titles SET status='published' WHERE id=p_title_a;
    RETURN QUERY SELECT 'admin_draft_to_published','FAIL','not denied';
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_draft_to_published','PASS',SQLERRM; END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'creator_submit','FAIL',SQLERRM; RETURN; END;

  -- 7. admin submitted -> published
  PERFORM set_config('request.jwt.claims',json_build_object('sub',p_admin::text,'role','authenticated')::text,true);
  BEGIN UPDATE public.content_titles SET status='published' WHERE id=p_title_a;
    RETURN QUERY SELECT 'admin_submitted_to_published','FAIL','not denied';
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_submitted_to_published','PASS',SQLERRM; END;

  -- 8. admin submitted -> changes_requested
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_submitted_to_changes_requested','FAIL',SQLERRM; END;

  -- 9. admin cr -> published
  BEGIN UPDATE public.content_titles SET status='published' WHERE id=p_title_a;
    RETURN QUERY SELECT 'admin_cr_to_published','FAIL','not denied';
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_cr_to_published','PASS',SQLERRM; END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_submitted_to_approved','FAIL',SQLERRM; RETURN; END;

  -- 11. approved -> published + stamp
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_published_to_draft','PASS',SQLERRM; END;

  -- 13. published -> approved
  BEGIN UPDATE public.content_titles SET status='approved' WHERE id=p_title_a;
    RETURN QUERY SELECT 'admin_published_to_approved','PASS','withdrawal';
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_published_to_approved','FAIL',SQLERRM; END;

  -- 14. rejected -> published
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'admin_rejected_to_published','PASS',SQLERRM; END;

  -- 15. workspace admin cross workspace
  PERFORM set_config('request.jwt.claims',json_build_object('sub',p_creator_a::text,'role','authenticated')::text,true);
  UPDATE public.content_titles SET title=title WHERE id=p_title_b;
  GET DIAGNOSTICS n = ROW_COUNT;
  RETURN QUERY SELECT 'ws_admin_A_edits_B',
    CASE WHEN n=0 THEN 'PASS' ELSE 'FAIL' END, format('rows=%s',n);
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'insert_as_published','PASS',SQLERRM; END;

  -- cleanup: reset tA back to draft
  BEGIN UPDATE public.content_titles SET status='approved' WHERE id=p_title_a;
...
REVOKE ALL ON FUNCTION public.__verify_content_title_pub_guard(uuid,uuid,uuid,uuid,uuid,uuid,uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.__verify_content_title_pub_guard(uuid,uuid,uuid,uuid,uuid,uuid,uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.__verify_content_title_pub_guard(uuid,uuid,uuid,uuid,uuid,uuid,uuid) TO authenticated, service_role;
