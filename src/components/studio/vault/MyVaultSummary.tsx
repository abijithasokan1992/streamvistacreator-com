  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border/50 bg-secondary/10 p-8 text-center">
        ...
        <h3 className="font-semibold">No active vault yet</h3>
        <p className="text-sm text-muted-foreground mt-1">Pick a storage class below to launch your Studio Vault.</p>
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-border/50 bg-secondary/10 p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {(["active_vault", "catalog_vault", "archive_vault"] as StorageClass[]).map((cls) => {
          ...
          return (
            <div key={cls} className={`rounded-xl border bg-gradient-to-b p-4 ${meta.accent}`}>
